...
`--session-probe` (requires `--remote`) runs claude headless inside a synthetic cage on the VPS (NoNewPrivs + a fresh user namespace, reproducing the root-managed Bash sandbox runtime) and confirms that exact Fieldwork client calls escape the inner cage while the managed Bash policy blocks compound exclusions. The operator-only probe mode rewrites harmless model-facing markers into fixed client inputs before Claude evaluates permissions, so model safety behavior does not need to generate hostile shell text. It is opt-in because it spends model tokens and needs a confirmed Claude login; run it after Claude CLI updates. The command can be silent for up to 180 seconds while the headless session runs.
